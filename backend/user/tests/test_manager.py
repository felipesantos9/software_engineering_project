...
class UserManagerTest(TestCase):
    ...
    @patch.object(
        UserManager,
        '_create_user',
        return_value=get_mock_user(password='mocked_hashed_password')
    )
    def test_sucess_create_user_and_call_function_with_correct_parameters(
        self, mock_create_user
    ):
        """
          Test create a valid user with success in create_user and call
          _create_user with correct parameters.
        """
    ...
    @patch.object(
        UserManager,
        '_create_user',
        return_value=get_mock_user(
            password='mocked_hashed_password',
            is_staff=True,
            is_superuser=True
        )
    )
    def test_sucess_create_superuser_and_call_function_with_correct_parameters(
        self, mock_create_user
    ):
        """
          Test create a valid user with success in create_superuser and call
          _create_user with correct parameters.
        """
    ...
    """
    @patch(
        "user.managers.make_password",
        return_value="mocked_hashed_password"
    )
    def test_sucess_create_user_and_call_functions_with_correct_parameters(
        self, mock_make_password
    ):
        """
          Test create a valid user with success in _create_user and call
        ...
        self.user_manager.model.assert_called_once_with(
